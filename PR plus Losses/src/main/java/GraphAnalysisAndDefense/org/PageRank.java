...
public class PageRank {
     ...
     */
    public PageRank(double[][] adjMatrix, double[] lossVector, double Wf, int maxIteration) {
        if (adjMatrix == null) {
            ...
            for (int nod : mapNodeToRank.keySet()) {
                ...
                // If the parents of the given nod have parents (in degree links) then do the following
                    ...
                    for (Map.Entry<Integer, Integer> incomeNode : givenNode.entrySet()) {
                        ...
                        totalRank = weightOfInDegreeEdges * mapNodeToRank.get(currentNode) / numOfOutDegreeEdges;
                    }
                    totalRank = (1 - dampingFactor) / numberOfNodes + dampingFactor * totalRank + Wf * assetLossVec[nod];
