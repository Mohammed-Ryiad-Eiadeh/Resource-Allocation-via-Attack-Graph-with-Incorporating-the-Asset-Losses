...
/**
 * This class is used to apply Genetic algorithm to find top-k paths between A and B
 * <pre>
 *     Genetic Algorithms, John H. Holland, 1992
 * </pre>
 */
public class GeneticAlgorithm extends FitnessFunction {
     ...
     * @param adjMatrix The adjacency matrix that represents the attack-defence graph
     ...
     */
    public GeneticAlgorithm(double[][] adjMatrix, double[] assetLosses, List<List<Integer>> population, double mutationRate, double crossOverProb, double selectionProportion, int maxIter) {
        super(adjMatrix, assetLosses);
        ...
        for (int i = 1; i < s.size() - 1; i++) {
            if (new Random().nextDouble() < mutationRate) {
                ...
                for (int j = 0; j < adjMatrix[0].length; j++) {
                    if (adjMatrix[s.get(i) - 1][j] > 0 && j + 1 != s.get(0) && j + 1 != s.get(s.size() - 1)) {
                        potentialsGens.add(j + 1);
                    }
                }
                for (int gene : potentialsGens) {
                    if (!s.contains(gene)) {
                        if (adjMatrix[s.get(i - 1) - 1][gene - 1] > 0 && adjMatrix[s.get(i + 1) - 1][gene - 1] > 0)
                            s.set(i, gene);
                    }
                }
            }
            ...
            for (int j = 1; j < solution2.size() - 1; j++) {
                if (!solution1.get(i).equals(solution2.get(j))) {
                    if (new Random().nextDouble() > crossOverProb) {
                        ...
                        List<Integer> s1 = new ArrayList<>(super.evaluateSolution(child1) >
                                super.evaluateSolution(solution1) ? child1 : solution1);
                        List<Integer> s2 = new ArrayList<>(super.evaluateSolution(child2) >
                                super.evaluateSolution(solution2) ? child2 : solution2);
                        solution1.clear();
                        solution2.clear();
                        solution1.addAll(s1);
                        solution2.addAll(s2);
